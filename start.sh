...
for var in KRATOS_INTERNAL HYDRA_INTERNAL KETO_INTERNAL TALOS_INTERNAL UI_INTERNAL OATHKEEPER_INTERNAL; do
  eval "value=\${$var}"
  if [ -z "$value" ]; then
...
envsubst '${PORT} ${KRATOS_INTERNAL} ${HYDRA_INTERNAL} ${UI_INTERNAL} ${UI_PORT} ${KETO_INTERNAL} ${TALOS_INTERNAL} ${DNS_RESOLVER} ${OATHKEEPER_INTERNAL}' \
